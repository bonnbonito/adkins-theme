<?php global $firmasite_settings; 

remove_action( 'close_footer', "firmasite_premium_menu_bottom_side_menu");
?>
<footer id="footer" class="site-footer clearfix" role="contentinfo">
    <div class="site-info <?php echo $firmasite_settings["layout_container_class"]; ?>">
        <?php do_action( 'open_footer' ); ?>
            <div class="row">
             <?php if ( ! dynamic_sidebar( 'footer-middle' ) ) : ?>
             <?php endif; // end sidebar widget area ?>
             </div>
               <?php if (has_nav_menu('footer_menu')) : 
               switch ($firmasite_settings["footer-menu-style"]) {
                    case "simple":
               ?>
                      <div class="hidden-lg">
                          <a class="navbar-toggle collapsed btn btn-default btn-sm" data-toggle="collapse" data-target=".footer-menu-collapse">
                            <span class="icon-reorder"></span>
                            <b class="caret"></b>
                          </a>
                      </div>                 
                      <nav id="nav-footer" class="collapse navbar-collapse footer-menu-collapse dropup" role="navigation">
                        <?php  wp_nav_menu(array('theme_location' => 'footer_menu', 'menu_class' => 'nav nav-pills')); ?>
                        <?php firmasite_premium_menu_bottom_side_menu(); ?>
                      </nav>
               <?php
                    break;
                   
                    case "default":
                    case "alternative":
                    default:
               ?>
               <div class="clearfix"></div>
                <nav id="footermenu" role="navigation"  class="site-navigation footer-navigation navbar dropup <?php if ((isset($firmasite_settings["alternative"]) && !empty($firmasite_settings["alternative"])) || "alternative" == $firmasite_settings["footer-menu-style"]){ echo " navbar-inverse";} else { echo " navbar-default"; } ?>">          
                  <div class="navbar-header">
                      <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target=".footer-menu-collapse">
                        <span class="sr-only"><?php _e("Toggle navigation", 'firmasite-base' );?></span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                      </button>
                  </div>               
                  <div id="nav-footer" class="collapse navbar-collapse footer-menu-collapse" role="navigation">
                    <?php  wp_nav_menu(array('theme_location' => 'footer_menu', 'menu_class' => 'nav navbar-nav')); ?>
					<?php do_action("firmasite_premium_logo_side_open"); ?>
                    <ul class="firmasite-premium-menu nav navbar-nav navbar-right dropup">
                        <?php do_action("firmasite_premium_menu_open"); ?>
                        
                        <?php do_action("firmasite_premium_menu_close"); ?>
                    </ul> 
                    <?php do_action("firmasite_premium_logo_side_close"); ?>
                 </div>
                </nav>    <!-- .site-navigation .footer-navigation --> 
               <?php 
                    break;
               }
               endif; ?>


        <?php do_action( 'close_footer' ); ?>
         <?php if($firmasite_settings["poweredby"]) { ?>
         <small class="text-muted generator"><a href="<?php echo esc_url( __( 'http://wordpress.org/', 'firmasite-base' ) ); ?>" title="<?php printf( esc_attr__( 'Proudly powered by %s', 'firmasite-base' ), 'WordPress' ); ?>"><i id="wplogo" class="icon-wordpress"></i></a></small>
         <?php } ?>
         <?php if($firmasite_settings["designer"]) { ?>
         <small class="text-muted designer"><?php _e("Theme:",'firmasite-base'); ?> <a href="//firmasite.com/" class="text-muted" rel="designer">FirmaSite</a></small>
         <?php } ?>
    </div><!-- .site-info -->
</footer><!-- #colophon .site-footer -->
